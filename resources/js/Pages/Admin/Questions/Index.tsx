...
interface Pregunta {
    id: number; escenario: string; contexto: string; opciones: any[];
...
export default function QuestionsIndex() {
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await fetch('/api/admin/entities/preguntas', { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) { const data = await res.json(); setPreguntas(data.data || []); }
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            await fetch(`/api/admin/entities/preguntas/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
                ...
                {preguntas.map((preg, index) => (
                    <div key={preg.id} className="bg-white rounded-xl p-5 border border-slate-200 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between mb-3">
                            ...
                            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${preg.activa ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
